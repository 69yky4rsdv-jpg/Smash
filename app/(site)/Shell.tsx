...
export default function SiteShell({
  ...
  const isStoreClipPage =
    pathname != null &&
    (pathname === "/store/success" || /^\/store\/[^/]+$/.test(pathname));
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header
        className={`border-b border-white/5 bg-black/60 backdrop-blur-xl pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)] ${
          isStoreClipPage ? "bg-black/90" : ""
        }`}
      >
        {isStoreClipPage ? (
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-3 py-2 sm:px-4">
            <Link
              href="/"
              className="text-sm font-bold uppercase tracking-[-0.03em] text-pink-200"
            ...
            <div className="flex items-center gap-3 text-xs">
              {isAdmin ? (
                <Link href="/admin" className="text-pink-300/90 hover:text-pink-200">
                  Admin
                </Link>
              ) : null}
              {isLoggedIn ? (
                <a href="/auth/logout" className="text-neutral-400 hover:text-neutral-200">
                  Logout
                </a>
              ) : (
                <Link href="/auth/login" className="text-neutral-400 hover:text-neutral-200">
                  Login
                </Link>
              )}
            </div>
      ...
      <footer
        className={`border-t border-white/5 bg-black/80 pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)] pb-[env(safe-area-inset-bottom)] ${
          isStoreClipPage ? "py-3" : ""
        }`}
      >
        <div
          className={`mx-auto flex max-w-6xl flex-col gap-3 px-4 text-xs text-neutral-500 ${
            isStoreClipPage ? "py-3" : "py-6"
          }`}
        >
